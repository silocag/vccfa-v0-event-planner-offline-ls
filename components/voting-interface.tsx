...
import type { Event, Location, ProposedDay, Guest } from "@/types/app"
import {
  Calendar,
  MapPin,
  Plus,
...
// Removed: import { EditAccommodationDialog } from "./edit-accommodation-dialog"
...
export function VotingInterface({ event, userRole }: VotingInterfaceProps) {
  const { user, updateEvent } = useAuth()
  const [selectedVoterGuestId, setSelectedVoterGuestId] = useState<string | null>(null) // Stores Guest.id
  const [showAddLocation, setShowAddLocation] = useState(false)
  // Removed: const [showAddAccommodation, setShowAddAccommodation] = useState(false)

  // Edit Location states
  const [showEditLocationDialog, setShowEditLocationDialog] = useState(false)
  const [selectedLocationForEdit, setSelectedLocationForEdit] = useState<Location | null>(null)

  // Removed: Edit Accommodation states
  // Removed: const [showEditAccommodationDialog, setShowEditAccommodationDialog] = useState(false)
  // Removed: const [selectedAccommodationForEdit, setSelectedAccommodationForEdit] = useState<Accommodation | null>(null)

  const [currentMonth, setCurrentMonth] = useState(new Date().getMonth())
  const [currentYear, setCurrentYear] = useState(new Date().getFullYear())

  // Form states (only for Location now)
  const [locationName, setLocationName] = useState("")
  const [locationAddress, setLocationAddress] = useState("")
  // Removed: const [accommodationName, setAccommodationName] = useState("")
  // Removed: const [accommodationDescription, setAccommodationDescription] = useState("")
  ...
  const getDaysInMonth = (year: number, month: number) => {
    ...
    const lastDayIndex =
      (firstDayIndex +
        days.filter((d) => d !== null).length -
        1 +
        days.filter((d) => d === null && days.indexOf(d) < days.findIndex((day) => day?.dayOfMonth === 1)).length) %
      7
  ...
  const addLocation = () => {
    ...
    updateEvent(event.id, { locations: [...event.locations, newLocation] })
    setLocationName("")
    setLocationAddress("")
  ...
  const handleSaveEditedLocation = (locationId: string, updates: Partial<Location>) => {
    const updatedLocations = event.locations.map((loc) => (loc.id === locationId ? { ...loc, ...updates } : loc))
  ...
  // Removed: addAccommodation
  // Removed: handleEditAccommodation
  // Removed: handleSaveEditedAccommodation
  // Removed: removeAccommodation

  const addProposedDay = (dateString: string) => {
    const newDay: ProposedDay = { id: Date.now().toString(), date: dateString }
  ...
  const getVoteCounts = (category: string, itemId: string) => {
    const categoryVotes = event.votes[category] || {}
    let available = 0,
      maybe = 0,
      notAvailable = 0
    Object.values(categoryVotes).forEach((guestVotes: any) => {
      const v = guestVotes[itemId]
      if (v === "available") available++
      else if (v === "maybe") maybe++
      else if (v === "not-available") notAvailable++
    })
    return { available, maybe, notAvailable }
  }

  const toggleAgreed = (category: "days" | "locations" | "accommodations", itemId: string) => {
    if (!canManage) return
    const agreedKey = `agreed${category.charAt(0).toUpperCase() + category.slice(1)}` as keyof Event
    const currentAgreed = (event[agreedKey] as string[] | undefined) ?? []
    ...
    updateEvent(event.id, { [agreedKey]: newAgreed } as Partial<Event>)
  }
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="mb-6 border rounded-lg p-4 bg-gray-50">
            ...
            <div className="grid grid-cols-7 gap-1">
              {calendarDays.map((day, index) => {
                ...
                return (
                  <div key={day.dateString || `day-${index}`} className={cellClasses} onClick={handleCellClick}>
                    ...
                    {canManage && !proposedDay && day.isCurrentMonth && (
                      <Button
                        size="xs" // Custom size or very small
                        variant="outline"
                        ...
                        {canManage && (
                          <>
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => toggleAgreed("days", day.id)}
                  ...
                  <Dialog open={showAddLocation} onOpenChange={setShowAddLocation}>
                    ...
                    <DialogContent>
                      <DialogHeader>
                        ...
                        <DialogDescription>Propose a location for the event</DialogDescription>
                      </DialogHeader>
                      <div className="space-y-4">
                        ...
                        <div className="flex justify-end space-x-2">
                          ...
                          <Button onClick={addLocation} disabled={!locationName.trim()}>
                            Add Location
                          </Button>
                        </div>
                      </div>
                    ...
                    {canManage && (
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem
                            onClick={() => toggleAgreed("locations", location.id)}
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              ...
                              <AlertDialogFooter>
                                <AlertDialogCancel>Cancel</AlertDialogCancel>
                                <AlertDialogAction onClick={() => removeLocation(location.id)}>
                                  Remove Location
                                </AlertDialogAction>
                              </AlertDialogFooter>
                            </AlertDialogContent>
      ...
      {/* Removed Accommodations section from here */}
      ...
      {/* Removed EditAccommodationDialog from here */}
    </div>
  )
}
