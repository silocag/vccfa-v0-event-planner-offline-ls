...
export function AuthProvider({ children }: { ReactNode }) {
  ...
  useEffect(() => {
    // Version check - clear old data if structure has changed
    const DATA_VERSION = "2.1" // Increment this when data structure changes
    const savedVersion = localStorage.getItem("event-planner-data-version")

    let shouldLoadFromStorage = true
    if (savedVersion !== DATA_VERSION) {
      console.log("Data structure version mismatch, clearing old data...")
      localStorage.removeItem("event-planner-events")
      localStorage.removeItem("event-planner-users")
      localStorage.removeItem("event-planner-current-user")
      localStorage.setItem("event-planner-data-version", DATA_VERSION)
      shouldLoadFromStorage = false
    }

    // Load data from localStorage
    const savedUsers = shouldLoadFromStorage ? localStorage.getItem("event-planner-users") : null
    const savedEvents = shouldLoadFromStorage ? localStorage.getItem("event-planner-events") : null
    const savedCurrentUser = shouldLoadFromStorage ? localStorage.getItem("event-planner-current-user") : null
    ...
    if (savedEvents) {
      ...
      const parsedEvents = JSON.parse(savedEvents).map((event: Event) => ({
        ...
        guests: event.guests.map((guest) => {
          ...
          return {
            ...
            assignedRoomId: guest.assignedRoomId || undefined,
            assignedBedId: guest.assignedBedId || undefined,
          }
        }),
          ...
          Object.entries(event.votes || {}).map(([category, userVotes]) => [
            ...
            Object.fromEntries(
              Object.entries(userVotes as { [userId: string]: any }).map(([userId, itemVotes]) => {
                ...
                return [guestForUser ? guestForUser.id : userId, itemVotes]
              }),
            ),
          ]),
        ),
      ...
      // Create demo events with comprehensive data - this is the full demo data from before
          ...
          guests: [
            {
              ...
              assignedRoomId: "room-1-1",
              assignedBedId: "bed-1-1-1",
            },
            {
              id: "guest-2",
              name: "Bob Smith",
              ...
              assignedRoomId: "room-2-1",
              assignedBedId: "bed-2-1-1",
            },
            {
              id: "guest-3",
              name: "Carol Davis",
              ...
              assignedRoomId: "room-1-2",
              assignedBedId: "bed-1-2-1",
            },
            {
              id: "guest-4",
              name: "David Wilson",
          ...
          activities: [
            {
              ...
              date: "2025-07-15",
              time: "09:00",
              assignedGuestIds: ["guest-1", "guest-2", "guest-3"],
            },
            {
              id: "act-2",
              name: "BBQ Dinner",
